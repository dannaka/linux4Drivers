...
static int scull_u_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_u_count && 
			(scull_u_owner != current->real_cred->uid.val) &&  /* allow user */
			(scull_u_owner != current->real_cred->euid.val) && /* allow whoever did su */
	...
	if (scull_u_count == 0)
		scull_u_owner = current->real_cred->uid.val; /* grab it */
...
static inline int scull_w_available(void)
{
	return scull_w_count == 0 ||
		scull_w_owner == current->real_cred->uid.val ||
		scull_w_owner == current->real_cred->euid.val ||
...
static int scull_w_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_w_count == 0)
		scull_w_owner = current->real_cred->uid.val; /* grab it */
	scull_w_count++;
	spin_unlock(&scull_w_lock);
